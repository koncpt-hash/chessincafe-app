    ...
    <div className="flex flex-col gap-4">
            <div className="flex justify-between items-center">
              ...
              <button
                onClick={onClose}
                aria-label="Zamknij"
                className="border border-border-strong size-[26px] text-sm text-muted hover:bg-surface-2"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex justify-between gap-4 items-start flex-wrap">
              <div>
                <h3 className="font-display text-2xl font-semibold text-ink">{eventTitle}</h3>
              ...
              <span className="text-[10px] px-2 py-1 border border-border-strong bg-surface-2 text-muted font-mono-data whitespace-nowrap">
                {cafe.difficultyLabel}
              </span>
            </div>

            <div className="flex items-center gap-2.5 border border-border bg-surface-2 px-3.5 py-2.5 text-xs text-muted flex-wrap">
              <span>
            ...
            <div className="grid grid-cols-2 gap-px bg-border border border-border">
              {[
                ...
                <div key={label} className="bg-surface p-3">
                  <span className="block text-[11px] uppercase tracking-wide text-faint font-semibold mb-1">
                    {label}
                  </span>
                  <b className="text-sm font-mono-data">{val}</b>
                </div>
              ))}
            </div>
            ...
            <div className="relative flex flex-col gap-1.5 pt-1 border-t border-border">
              ...
              <button
                ...
                className="text-sm text-muted underline underline-offset-2 text-left"
              >
                Wolisz grać regularnie? Zobacz cały sezon w tej kawiarni →
              </button>
              {seasonOpen && (
                <div className="text-xs text-muted border border-border p-3.5 mt-1.5">
                  <span className="text-[11px] uppercase tracking-wide text-faint font-semibold">
            ...
            <p className="text-sm text-muted max-w-[62ch]">{cafe.desc}</p>

            <div className="border border-border p-3.5">
              <h4 className="text-xs uppercase tracking-wide text-faint font-semibold mb-2">
                Kto już gra
              </h4>
              <div className="flex flex-wrap gap-2">
                {cafe.players.length ? (
                  cafe.players.map((p) => (
                    <span
                      key={p}
                      className="flex items-center gap-1.5 text-xs bg-surface-2 border border-border pr-2.5 pl-1"
                    >
                      <span className="size-[22px] rounded-full bg-surface border border-border-strong flex items-center justify-center font-mono-data text-[9px] font-semibold">
                        {initials(p)}
                      </span>
                      {p}
                    </span>
                  ...
                  <span className="text-[11px] text-faint">Bądź pierwszym zapisanym na tę rundę</span>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex justify-between items-baseline gap-2.5 text-xs text-muted flex-wrap">
